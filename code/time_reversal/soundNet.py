'''
Load all the audio files into the memory and then process batches
'''
...
class soundNet():
	...
	def get_datasets(self):
		...
		for file_num, file in enumerate(self.filenames):
			print(f'Loading {file_num+1}/{len(self.filenames)+1} file into the memory', end="\r")
			waveform, fs_ = librosa.load((self.root_dir + file), sr = self.sr)
			self.waveforms.append(waveform)
			if file_num >= 8:
				break
		
		indices = np.arange(0, len(self.waveforms), 1)
		random.seed(random_state)
		random.shuffle(indices)
		train_index  = int((1 - self.val_split) * len(self.waveforms))

		self.train_waveforms = np.array(self.waveforms)[indices[:train_index]]
		self.val_waveforms = np.array(self.waveforms)[indices[train_index:]]

		train_dataset = fmaDataset(self.device, self.train_waveforms, transform_prob = self.transform_prob)
		val_dataset = fmaDataset(self.device, self.val_waveforms, transform_prob = self.transform_prob)
...
class fmaDataset(Dataset):
	...
	def __init__(self, device, waveforms, transform_prob):
	...
	def __getitem__(self, idx):
		...
		if reversal_prob >= self.transform_prob:
			waveform = np.flip(self.waveforms[idx])
